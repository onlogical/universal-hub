...
import { Box, ScrollArea, Tabs, Text, ThemeProvider } from "@prism";
...
export function UniversalHubMenu({ model }: { readonly model: UniversalHubMenuModel }): React.ReactElement {
	...
	const tabs = model.pages.map((page) => ({ value: page.id, label: page.label, icon: page.icon }));
							...
							styleOverrides={{
								...
								tab: (_styles, context) => ({
									...
									backgroundTransparency:
										context.state === "selected"
											? 0.76
											: context.state === "hovered" || context.state === "focused"
												? 0.62
												: 1,
									strokeTransparency: 1,
									textColor:
