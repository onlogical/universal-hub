...
function partIsAccessoryOwned(part: Instance, dummy: Model): boolean {
...
function stylePreviewChamsOverlay(clone: BasePart, color: Color3, transparency: number, inflate: number) {
	clone.Material = Enum.Material.SmoothPlastic;
	clone.Color = color;
	clone.Transparency = transparency;
	clone.Size = clone.Size.mul(inflate);
	...
	clone.Massless = true;
	if (clone.IsA("MeshPart")) clone.TextureID = "";
	for (const surface of clone.GetDescendants()) {
		if (surface.IsA("JointInstance") || surface.IsA("WeldConstraint")) surface.Destroy();
		else if (surface.IsA("SpecialMesh")) {
			surface.TextureId = "";
			surface.Scale = surface.Scale.mul(inflate);
		} else if (surface.IsA("Decal") || surface.IsA("Texture") || surface.IsA("SurfaceAppearance"))
			surface.Destroy();
	}
}

function weldPreviewOverlay(source: BasePart, clone: BasePart, parent: Instance) {
...
function attachPreviewHitboxes(dummy: Model, parent: Instance, outlineColor: Color3, excludeAccessories: boolean) {
	for (const descendant of dummy.GetDescendants()) {
		if (!descendant.IsA("BasePart") || descendant.Transparency >= 1 || descendant.Name === "HumanoidRootPart")
			continue;
		if (partIsAccessoryOwned(descendant, dummy) && excludeAccessories) continue;
		const box = new Instance("SelectionBox");
		box.Name = `${descendant.Name}_Hitbox`;
...
function ViewportDummy({
	...
	// ViewportFrames do not composite Roblox Highlights. Hide the dummy's
	// drawn surfaces and weld an unlit fill overlay so Chams cannot Z-fight.
	// Hitboxes use SelectionBox strokes; a cloned outline shell would read as a fill.
	React.useEffect(() => {
		if (scene === undefined || preview?.worldRenderer !== "native" || preview.chams !== true) return;
		const overlay = new Instance("Folder");
		overlay.Name = "UniversalHubPreviewChams";
		overlay.Parent = scene.world;
		const restore = new Array<() => void>();
		const chamsColor = preview.chamsColor ?? Color3.fromRGB(255, 118, 87);
		const fillTransparency = math.min(preview.chamsTransparency ?? 0.42, 0.22);
		for (const descendant of scene.dummy.GetDescendants()) {
			if (!descendant.IsA("BasePart") || descendant.Transparency >= 1 || descendant.Name === "HumanoidRootPart")
				continue;
			if (partIsAccessoryOwned(descendant, scene.dummy) && preview.chamsExcludeAccessories === true) continue;
			const localTransparency = descendant.LocalTransparencyModifier;
			descendant.LocalTransparencyModifier = 1;
			restore.push(() => {
				if (descendant.Parent !== undefined) descendant.LocalTransparencyModifier = localTransparency;
			});
			const fill = descendant.Clone();
			fill.Name = `${descendant.Name}_ChamsFill`;
			stylePreviewChamsOverlay(fill, chamsColor, fillTransparency, 1.035);
			weldPreviewOverlay(descendant, fill, overlay);
		}
		return () => {
	...
	React.useEffect(() => {
		...
		const overlay = new Instance("Folder");
		overlay.Name = "UniversalHubPreviewHitboxes";
		overlay.Parent = scene.world;
		attachPreviewHitboxes(
			scene.dummy,
			overlay,
			preview.outlineColor ?? Color3.fromRGB(255, 118, 87),
			preview.chamsExcludeAccessories === true,
		);
		return () => {
			overlay.Destroy();
		};
	...
	return (
		<React.Fragment>
			<viewportframe
				...
				ref={setViewport}
				Ambient={
					preview?.worldRenderer === "native" && preview.chams === true
...
function EspTargetColorPicker({
	...
	return (
		<ColorPicker
			...
			zIndex={100}
			styleOverrides={(_styles, ctx) => ({
				panelBackgroundColor: ctx.theme.colors.background.raised,
				raisedBackgroundColor: ctx.theme.colors.background.surface,
				panelStrokeColor: ctx.theme.colors.border.default,
				panelStrokeTransparency: 0.15,
				fieldStrokeColor: ctx.theme.colors.border.subtle,
			})}
			slotProps={{
				trigger: { BackgroundTransparency: 1 },
				triggerStroke: { Transparency: 1 },
				triggerLabel: { Visible: false },
				triggerSwatch: { Position: UDim2.fromOffset(0, 0), Size: UDim2.fromScale(1, 1), strokeTransparency: 1 },
				alphaCheckerboard: { Visible: fill },
			}}
		/>
	);
}
					...
					{relationship.targets.map((target) => {
						...
						return (
							<frame
								...
								BackgroundColor3={selected ? Color3.fromRGB(38, 38, 42) : Color3.fromRGB(31, 31, 34)}
...
function WhatsNewModal({
	...
	const visible = notice?.visible === true;
	...
	return (
		...
		>
			<Stack width="100%" gap="md">
				<Text
					text={notice?.releases?.[0]?.displayVersion ?? current}
					size="sm"
					weight={700}
					color={theme.text.disabled}
					width="100%"
									...
									return (
										<Stack key={section.id} width="100%" gap="xs">
											<Text
												...
												slotProps={{ root: { TextXAlignment: Enum.TextXAlignment.Left } }}
											/>
											{(section.groups ?? [{ tab: "", items: section.items.map((item) => ({ name: item })) }]).map((group) => (
												<Stack key={group.tab || group.items[0]?.name || section.id} width="100%" gap="xs">
													...
													{group.items.map((feature) => (
														<Text
															key={feature.name}
															text={feature.note ? `${feature.name} - ${feature.note}` : feature.name}
															size="sm"
															weight={600}
															color={theme.text.secondary}
															width="100%"
				...
				<Stack width="100%" direction="horizontal" gap="sm">
					<Button
						...
						variant="outline"
						color="primary"
						onPress={() => onAction?.("whatsNewDontShowAgain")}
					/>
					<Button
						label="View all"
						variant="outline"
						color="primary"
						onPress={() => onAction?.("whatsNewViewAll")}
					/>
					<Button
						label="Got it"
						variant="filled"
						color="primary"
						onPress={() => onAction?.("whatsNewAcknowledge")}
					/>
				</Stack>
			</Stack>
		</Modal>
