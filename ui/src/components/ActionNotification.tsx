...
function placement(position: ActionNotificationModel["position"]): {
	...
	switch (position) {
		...
		case "bottom-left":
			return { anchor: new Vector2(0, 1), start: new UDim2(0, margin - 18, 1, -bottomMargin), target: new UDim2(0, margin, 1, -bottomMargin) };
		case "bottom-center":
			return { anchor: new Vector2(0.5, 1), start: new UDim2(0.5, 0, 1, -bottomMargin + 18), target: new UDim2(0.5, 0, 1, -bottomMargin) };
		case "bottom-right":
			return { anchor: new Vector2(1, 1), start: new UDim2(1, -margin + 18, 1, -bottomMargin), target: new UDim2(1, -margin, 1, -bottomMargin) };
		default:
...
export function ActionNotification({
	...
	return (
		...
		>
			...
			<uipadding PaddingTop={new UDim(0, 14)} PaddingBottom={new UDim(0, 12)} PaddingLeft={new UDim(0, 16)} PaddingRight={new UDim(0, 16)} />
			<textbutton
			...
			<Stack width="100%" height="100%" justify="spaceBetween">
				...
				<Stack width="100%" direction="horizontal" justify="end" gap="xs">
					<Button label="Not Now" variant="outline" color="primary" size="sm" width={92} onPress={() => close(onDismiss)} />
					<Button
