...
function TaskPolicy.snapshot(taskLibrary, playerDataController)
    if type(playerDataController) == "table" and playerDataController.TASKS_DATA_NAMES
    ...
    for _, group in ipairs(groups) do
        ...
        if type(records) == "table" then
            ...
            for index, record in ipairs(records) do
                ...
                if normalized then table.insert(result, normalized); added[index] = true end
            end
            local keys = {}
            for key, _ in pairs(records) do
                if not added[key] and type(key) ~= "number" then table.insert(keys, key) end
            end
...
local RANGE_FAMILY = {
    Beginner1 = "range", Beginner2 = "targets", Beginner3 = "slide",
    Beginner4 = "equip", Beginner5 = "dash", Beginner6 = "grenade",
}
...
local function pvpFamily(name, title)
    ...
    if beginner then return beginner end
    local text = string.lower((name or "") .. " " .. (title or ""))
    if string.find(text, "eliminat", 1, true) or string.find(text, " kill", 1, true) then return "eliminations" end
    if string.find(text, "streak", 1, true) then return "streaks" end
    if string.find(text, "win", 1, true) then return "wins" end
    if string.find(text, "round", 1, true) then return "rounds" end
    if string.find(text, "duel", 1, true) or string.find(text, "match", 1, true) or string.find(text, "play", 1, true) then return "play" end
    return nil
end
...
function TaskPolicy.classify(task)
    ...
    if rangeFamily then return { kind = "range", type = "range", family = rangeFamily, supported = true, requiresCombat = false } end
    ...
    if family then
        ...
        local variant = string.find(lower, "event", 1, true) and "event"
            ...
            or nil
        return {
            kind = "pvp", type = "pvp", family = family, variant = variant,
            scope = variant, supported = true, requiresCombat = true,
            event = variant == "event", limited = variant == "limited",
        }
    end
...
function TaskPolicy.select(tasks)
    ...
    for _, task in ipairs(tasks or {}) do
        ...
        if task.completed ~= true and task.Completed ~= true and not task.locked
            and classification and classification.supported
        then
