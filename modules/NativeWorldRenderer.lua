...
local NativeWorldRenderer = {}
NativeWorldRenderer.__index = NativeWorldRenderer

-- A hard cap keeps adversarial character hierarchies from producing unbounded Instances.
NativeWorldRenderer.HIGHLIGHT_BUDGET = 255
NativeWorldRenderer.SUBJECT_HIGHLIGHT_BUDGET = 17
NativeWorldRenderer.HIGHLIGHT_DISTANCE = 1000
NativeWorldRenderer.HIGHLIGHT_DISTANCE_HYSTERESIS = 100
NativeWorldRenderer.DISTANCE_REFRESH_INTERVAL = 0.25

local COLORS = NativeVisualPolicy.COLORS
...
function NativeWorldRenderer.new(context)
    assert(context and context.guiParent, "NativeWorldRenderer requires guiParent")
    assert(context.store, "NativeWorldRenderer requires a store")
    assert(context.players, "NativeWorldRenderer requires Players")
    assert(context.runService and context.runService.Heartbeat,
        "NativeWorldRenderer requires RunService.Heartbeat")

    local self = setmetatable({
        context = context,
        create = context.createInstance or Instance.new,
        policy = {},
        subjects = {},
        extraKeys = {},
        playerConnections = {},
...
function NativeWorldRenderer:_settings()
    local state = self.context.store:Get()
    return state.settings or {}
end

function NativeWorldRenderer:_ensureRoot()
    if self.root then
        return self.root
    end
...
function NativeWorldRenderer:_playerTone(player, character)
...
function NativeWorldRenderer:_eligible(player, character)
...
function NativeWorldRenderer:_newHighlight(subject, adornee)
    if self.highlightCount >= NativeWorldRenderer.HIGHLIGHT_BUDGET
...
function NativeWorldRenderer:_clearHighlights(subject)
...
function NativeWorldRenderer:_configureHighlights(subject)
    ...
    if descendantAdded then
        table.insert(subject.hierarchyConnections, descendantAdded:Connect(rebuild))
    end
    if descendantRemoving then
        table.insert(subject.hierarchyConnections, descendantRemoving:Connect(rebuild))
    end
    rebuild()
end
...
function NativeWorldRenderer:_refreshHighlightDistances()
    if not self:_perPartRelevant(self:_settings()) then
        return
    end
    local localRoot = self:_localRootPart()
    local enterDistanceSquared = NativeWorldRenderer.HIGHLIGHT_DISTANCE ^ 2
    local exitDistanceSquared = (NativeWorldRenderer.HIGHLIGHT_DISTANCE
        + NativeWorldRenderer.HIGHLIGHT_DISTANCE_HYSTERESIS) ^ 2
    local candidates = {}
    ...
    local poolCount = math.min(#candidates, NativeWorldRenderer.HIGHLIGHT_BUDGET)
    local baseBudget = poolCount > 0 and math.min(
        NativeWorldRenderer.SUBJECT_HIGHLIGHT_BUDGET,
        math.floor(NativeWorldRenderer.HIGHLIGHT_BUDGET / poolCount)
    ) or 0
    local remainder = baseBudget < NativeWorldRenderer.SUBJECT_HIGHLIGHT_BUDGET
        and NativeWorldRenderer.HIGHLIGHT_BUDGET - baseBudget * poolCount or 0
    local budgets = {}
...
function NativeWorldRenderer:_stopDistanceRefresh()
    disconnect(self.distanceConnection)
    self.distanceConnection = nil
    self.distanceAccumulator = 0
end

function NativeWorldRenderer:_syncDistanceRefresh()
    if not self:_perPartRelevant(self:_settings()) then
    ...
    self.distanceConnection = self.context.runService.Heartbeat:Connect(function(deltaTime)
        if not self:_perPartRelevant(self:_settings()) then
            self:_stopDistanceRefresh()
            return
        end
        self.distanceAccumulator += deltaTime
        if self.distanceAccumulator < NativeWorldRenderer.DISTANCE_REFRESH_INTERVAL then
            return
        end
        self.distanceAccumulator %= NativeWorldRenderer.DISTANCE_REFRESH_INTERVAL
        self:_refreshHighlightDistances()
    end)
end

function NativeWorldRenderer:_rebalanceHighlights()
...
function NativeWorldRenderer:_styleHighlights(subject, color)
    local settings = self:_settings()
    color = color or subject.highlightColor or COLORS.danger
    subject.highlightColor = color
    local outlineColor = EspColorPolicy.color(settings, "outline", color, subject.tone)
    local fillColor = EspColorPolicy.color(settings, "fill", color, subject.tone)
    local fillAlpha = EspColorPolicy.fillAlpha(settings, 1 - NativeVisualPolicy.FILL_TRANSPARENCY, subject.tone)
    for _, highlight in ipairs(subject.highlights) do
...
function NativeWorldRenderer:_connectCharacterPolicy(subject)
    clearConnections(subject.policyConnections)
    local connector = subject.player and self.policy.connectCharacterChanged
    if connector then
        hold(subject.policyConnections, connector(subject.player, subject.character, function()
...
function NativeWorldRenderer:_makeSubject(key, descriptor, player)
    ...
    billboard.Size = UDim2.fromScale(5, 7)
    ...
    if humanoid then
        if humanoid.HealthChanged then
            table.insert(subject.connections, humanoid.HealthChanged:Connect(function()
                self:_updateHealth(subject)
            end))
        elseif humanoid.GetPropertyChangedSignal then
            table.insert(subject.connections, humanoid:GetPropertyChangedSignal("Health"):Connect(function()
                self:_updateHealth(subject)
            end))
        end
        if humanoid.GetPropertyChangedSignal then
            table.insert(subject.connections, humanoid:GetPropertyChangedSignal("MaxHealth"):Connect(function()
                self:_updateHealth(subject)
            end))
        end
    end
...
function NativeWorldRenderer:_removeSubject(key)
...
function NativeWorldRenderer:_weapon(subject, observation)
...
function NativeWorldRenderer:_updateHealth(subject)
    if not self.active or not self.subjects[subject.key] then
        return
    end
    ...
    local color = EspColorPolicy.healthColor(self:_settings(), fraction, COLORS.danger, COLORS.signal, subject.tone)
...
function NativeWorldRenderer:_updateSubject(subject, observation)
    local settings = self:_settings()
    ...
    subject.name.TextColor3 = EspColorPolicy.color(settings, "name", color, tone)
    ...
    subject.weapon.TextColor3 = EspColorPolicy.color(settings, "weapon", color, tone)
...
function NativeWorldRenderer:_attachCharacter(player, character)
    self:_removeSubject(player)
    self:_makeSubject(player, { character = character }, player)
end

function NativeWorldRenderer:_invalidatePlayer(player)
    if not self.active then
        return
    end
...
function NativeWorldRenderer:_connectPlayerPolicy(player)
    ...
    if self.policy.connectPlayerChanged then
        hold(connections, self.policy.connectPlayerChanged(player, function()
            self:_invalidatePlayer(player)
        end))
    end
end

function NativeWorldRenderer:_trackPlayer(player)
...
function NativeWorldRenderer:_untrackPlayer(player)
    self:_removeSubject(player)
...
function NativeWorldRenderer:_invalidatePolicy()
    if not self.active then
        return
    end
    local players = {}
...
function NativeWorldRenderer:_subscribePolicyChanged()
    if self.policy.subscribeChanged then
        hold(self.policyConnections, self.policy.subscribeChanged(function()
            self:_invalidatePolicy()
        end))
    end
end

function NativeWorldRenderer:_subscribeExtras()
    local subscribe = self.policy.subscribeExtras
    if not subscribe then
        return
    end
    self.extraCleanup = subscribe(function(descriptor)
...
function NativeWorldRenderer:_clearExtras()
    local keys = {}
...
function NativeWorldRenderer:_activate()
    if self.active then
        return
    end
    self.active = true
...
function NativeWorldRenderer:_deactivate()
    if not self.active then
        return
    end
    self.active = false
...
function NativeWorldRenderer:_applyState(state)
    if self.destroyed then
        return
    end
    local native = self.context.nativeWorldSupported ~= false
...
function NativeWorldRenderer:setPolicy(policy)
...
function NativeWorldRenderer:render(observations)
    local unclaimed = {}
...
function NativeWorldRenderer:destroy()
    if self.destroyed then
        return
    end
    self.destroyed = true
