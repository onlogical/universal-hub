...
function Catalog:info(sectionId, id, label, resolve, tone)
    local group = assert(self.groupById[sectionId], "Unknown presentation section: " .. tostring(sectionId))
    table.insert(group.infos, { id = id, label = label, resolve = resolve, tone = tone })
    self:_markPage(group.page)
end
...
function Catalog:model(state)
    ...
    for _, group in ipairs(self.groups) do
        if #group.options > 0 or #group.infos > 0 or #group.keybinds > 0 then
            local controls = {}
            for _, info in ipairs(group.infos) do
                local value = type(info.resolve) == "function" and info.resolve(state) or info.resolve
                append(controls, {
                    id = info.id,
                    kind = "info",
                    label = info.label,
    ...
    for _, page in ipairs(PAGE_ORDER) do
        if self.hasContent[page] then
            ...
            if metadata.preview then
                ...
                preview.tone = "enemy"
                local nativePolicy = self.context.nativeVisualPolicy
                local defaultAlpha = preview.worldRenderer == "native"
                    and (1 - (nativePolicy and nativePolicy.FILL_TRANSPARENCY or 0.42))
                    or 0.18
                local defaults = {
                ...
                local function relationshipPalette(relationship, label)
                    ...
                    local fillAlpha = EspColorPolicy.fillAlpha(settings, defaultAlpha, relationship)
                    return {
                        id = relationship,
                        label = label,
                        fillAlpha = fillAlpha,
                        targets = {
                            { id = "outline", label = "Outline", color = EspColorPolicy.color(settings, "outline", primary, relationship), defaultColor = primary },
                            { id = "fill", label = "Fill", color = EspColorPolicy.color(settings, "fill", primary, relationship), alpha = fillAlpha, defaultColor = primary, defaultAlpha = defaultAlpha },
                            { id = "name", label = "Name", color = EspColorPolicy.color(settings, "name", primary, relationship), defaultColor = primary },
                            { id = "weapon", label = "Weapon", color = EspColorPolicy.color(settings, "weapon", defaults.weapon, relationship), defaultColor = defaults.weapon },
                            { id = "healthLow", label = "Health Low", color = EspColorPolicy.color(settings, "healthLow", defaults.healthLow, relationship), defaultColor = defaults.healthLow },
                            { id = "healthHigh", label = "Health High", color = EspColorPolicy.color(settings, "healthHigh", defaults.healthHigh, relationship), defaultColor = defaults.healthHigh },
                        },
                    }
                end
    ...
    return {
        ...
        onValueChange = function(id, value, persist)
            ...
            elseif id:sub(1, 9) == "espColor:" then
                ...
                local setting = relationship and EspColorPolicy.settingName(relationship, target)
            ...
            elseif id:sub(1, 9) == "espAlpha:" then
                ...
                local setting = relationship and EspColorPolicy.settingName(relationship, "fillAlpha")
                if setting then
            ...
            elseif id:sub(1, 21) == "resetEspRelationship:" then
                ...
                for _, target in ipairs(EspColorPolicy.TARGETS) do
                    self.context.setSetting(EspColorPolicy.settingName(relationship, target), "", persist == true)
                end
                self.context.setSetting(EspColorPolicy.settingName(relationship, "fillAlpha"), -1, persist == true)
            elseif id == "resetEspAll" then
                for _, relationship in ipairs(EspColorPolicy.RELATIONSHIPS) do
                    for _, target in ipairs(EspColorPolicy.TARGETS) do
                        self.context.setSetting(EspColorPolicy.settingName(relationship, target), "", persist == true)
                    end
                    self.context.setSetting(EspColorPolicy.settingName(relationship, "fillAlpha"), -1, persist == true)
